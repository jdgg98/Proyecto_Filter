...
//El archivo original fue editado por Andrés Corrales Vargas - 2019, para quitar métodos no
//relevantes para el programa y que afectaban el tamaño del ejecutable, así como para
//la traducción de los errores que podrían generarse provenientes de este código.
...
public:
   ...
   void save_image(const std::string& file_name) const
   {
      ...
      if (!stream)
      {
         std::cerr << "Error 6: (Error BMP) No se pudo abrir el archivo "  << file_name << " para su escritura. Abortando" << std::endl;
         return;
      }
...
private:
   ...
   void load_bitmap()
   {
      ...
      if (!stream)
      {
         std::cerr << "ERROR 7: (Error BMP) El archivo BMP " << file_name_ << " no se encontró" << std::endl;
         return;
      }
      ...
      if (bfh.type != 19778)
      {
         ...
         std::cerr << "ERROR 8: (Error BMP) Valor de tipo inválido " << bfh.type << " se esperaba 19778. Abortando..." << std::endl;
         return;
      }
      ...
      if (bih.bit_count != 24)
      {
         ...
         std::cerr << "ERROR 9: (Error BMP) Profundidad de bits inválida (" << bih.bit_count << "). Se esperaba 24 bits. Abortando..." << std::endl;
      ...
      if (bih.size != bih.struct_size())
      {
         ...
         std::cerr << "ERROR 10: (Error BMP) Tamaño de BIH inválido " << bih.size << " se esperaba " << bih.struct_size() << ". Abortando..." << std::endl;
      ...
      if (bitmap_file_size != bitmap_logical_size)
      {
         ...
         std::cerr << "ERROR 11: (Error BMP) No existe igualdad entre los tamaños lógico y físico del mapa de bits. " <<
                      "Lógico: "  << bitmap_logical_size << " " <<
                      "Físico: " << bitmap_file_size    << ". Abortando..." << std::endl;
